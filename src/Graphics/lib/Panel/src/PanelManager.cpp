...
void PanelManager::draw(sf::RenderTarget& target, sf::RenderStates states) const {
    ...
    for (const auto [panel, position] : panels) {
        ...
        case Position::TOP_LEFT: {
            ...
            const auto new_tl_limit =
                    sf::Vector2f{std::max(top_left_limit.x, panel_size.x + PANEL_MARGIN),
                            offset.y + panel_size.y};

            if (new_tl_limit.x > top_right_limit.x || new_tl_limit.y > bottom_left_limit.y
        ...
        case Position::TOP_RIGHT: {
            offset = sf::Vector2f(window_size.x - panel_size.x - PANEL_MARGIN,
                    PANEL_MARGIN + top_right_limit.y);
            const auto new_tr_limit = sf::Vector2f{
                    std::min(top_right_limit.x, offset.x - PANEL_MARGIN), offset.y + panel_size.y};

            if (new_tr_limit.x < top_left_limit.x || new_tr_limit.y > bottom_right_limit.y
        ...
        case Position::BOTTOM_LEFT: {
            ...
            const auto new_bl_limit =
                    sf::Vector2f{std::max(bottom_left_limit.x, panel_size.x + PANEL_MARGIN),
                            offset.y - PANEL_MARGIN};

            if (new_bl_limit.x > bottom_right_limit.x || new_bl_limit.y < top_left_limit.y
                    || (new_bl_limit.x > top_right_limit.x && new_bl_limit.y < top_right_limit.y)) {
                continue;
            }
        ...
        case Position::BOTTOM_RIGHT: {
            offset = sf::Vector2f(window_size.x - panel_size.x - PANEL_MARGIN,
                    bottom_right_limit.y - panel_size.y - PANEL_MARGIN);
            const auto new_br_limit =
                    sf::Vector2f{std::min(bottom_right_limit.x, offset.x - PANEL_MARGIN),
                            offset.y - PANEL_MARGIN};

            if (new_br_limit.x < bottom_left_limit.x || new_br_limit.y < top_right_limit.y
                    || (new_br_limit.x < top_left_limit.x && new_br_limit.y < top_left_limit.y)) {
                continue;
            }
